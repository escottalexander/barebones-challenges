...
contract DeadMansSwitchTest is Test {
    ...
    uint256 ONE_THOUSAND = 1000 wei;
    uint256 INTERVAL = 1 weeks;
    uint256 HONEY_POT = 1 ether;
    ...
    function testDeposit() public {
        ...
        uint256 balance = deadMansSwitch.balanceOf(THIS_CONTRACT);
    ...
    function testSetCheckInInterval() public {
        ...
        uint256 checkInInterval = deadMansSwitch.checkInInterval(THIS_CONTRACT);
    ...
    function testWithdraw() public {
        ...
        deadMansSwitch.deposit{ value: ONE_THOUSAND }();
        uint256 balanceBefore = deadMansSwitch.balanceOf(NON_CONTRACT_USER);
        deadMansSwitch.withdraw(NON_CONTRACT_USER, ONE_THOUSAND);
        uint256 balanceAfter = deadMansSwitch.balanceOf(NON_CONTRACT_USER);
    ...
    function testWithdrawInsufficientBalance() public {
        ...
        vm.startPrank(NON_CONTRACT_USER);
        deadMansSwitch.deposit{ value: ONE_THOUSAND }();
        uint256 balance = deadMansSwitch.balanceOf(NON_CONTRACT_USER);
        assertEq(balance, ONE_THOUSAND);
        vm.expectRevert();
        deadMansSwitch.withdraw(NON_CONTRACT_USER, ONE_THOUSAND + 1);
        uint256 balance2 = deadMansSwitch.balanceOf(NON_CONTRACT_USER);
    ...
    function testWithdrawTransferFailed() public {
        ...
        uint256 balance2 = deadMansSwitch.balanceOf(THIS_CONTRACT);
        assertEq(balance2, ONE_THOUSAND);
    }
    ...
    function testWithdrawAsBeneficiary() public {
        ...
        uint256 initialBalance = address(BENEFICIARY_1).balance;
        deadMansSwitch.withdraw(THIS_CONTRACT, ONE_THOUSAND);
        uint256 finalBalance = address(BENEFICIARY_1).balance;
    ...
    function testWithdrawAsBeneficiaryTransferFailed() public {
        ...
        uint256 initialBalance = address(THIS_CONTRACT).balance;
        vm.expectRevert();
        deadMansSwitch.withdraw(BENEFICIARY_1, ONE_THOUSAND);
        uint256 finalBalance = address(THIS_CONTRACT).balance;
        assertEq(finalBalance, initialBalance);
        uint256 balance = deadMansSwitch.balanceOf(BENEFICIARY_1);
        assertEq(balance, ONE_THOUSAND);
    }
    ...
    function testContractCanHandleReceivedFundsWithoutCallData() public {
        vm.deal(NON_CONTRACT_USER, ONE_THOUSAND);
        vm.startPrank(NON_CONTRACT_USER);
        (bool success,) = payable(deadMansSwitch).call{ value: ONE_THOUSAND }("");
        assertTrue(success);
        uint256 balance = deadMansSwitch.balanceOf(NON_CONTRACT_USER);
        assertEq(balance, ONE_THOUSAND);
    }
    ...
    function testContractCanHandleReceivedFundsWithUnknownCallData() public {
        vm.deal(NON_CONTRACT_USER, ONE_THOUSAND);
        vm.startPrank(NON_CONTRACT_USER);
        (bool success,) = payable(deadMansSwitch).call{ value: ONE_THOUSAND }("0x1234");
        assertTrue(success);
        uint256 balance = deadMansSwitch.balanceOf(NON_CONTRACT_USER);
        assertEq(balance, ONE_THOUSAND);
    }
    ...
    function testReentrancyAttackFails() public {
        ...
        // It is ok if the exploit fails, we just want to make sure it doesn't work
...
contract ReentrancyTest {
    ...
    function setUp(address dmsContract) public payable {
