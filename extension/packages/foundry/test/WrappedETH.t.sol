...
contract WrappedETHTest is Test {
    ...
    function testFallback() public {
        (bool success,) = address(wrappedETH).call{ value: 1 ether }("");
        assertTrue(success);
        assertEq(wrappedETH.balanceOf(THIS_CONTRACT), 1 ether);
    }
    ...
    function testReentrancyAttackShouldFail() public {
        ...
        // Try to exploit the WrappedETH contract, it's ok if the call fails
        try reentrancyTest.exploitWithdraw() { } catch { }
        // If the exploit worked then the exploiter should have more than they put in
