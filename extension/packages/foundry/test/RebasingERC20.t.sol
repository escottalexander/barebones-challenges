...
contract RebasingERC20Test is Test {
  ...
  function testRebasePositive() public {
    ...
    assertEq(
      token.balanceOf(luffy), luffyBalance1 * (newTotalSupply) / oldTotalSupply
    );
    assertEq(
      token.balanceOf(zoro), (zoroBalance1 * newTotalSupply) / oldTotalSupply
    );
  }
   ...
   */
  function testTransferAfterRebase(
    uint256 transferAmount
  ) public {
    transferAmount = bound(transferAmount, 1e18, 10000e18);
    int256 supplyDelta = -9_000_000e18;
    uint256 luffyBalanceBefore1 = token.balanceOf(luffy);
    uint256 zoroBalanceBefore = token.balanceOf(zoro);
    token.transfer(zoro, transferAmount);
    uint256 luffyBalanceAfter1 = token.balanceOf(luffy);
    uint256 zoroBalanceAfter = token.balanceOf(zoro);
    assertEq(luffyBalanceAfter1, luffyBalanceBefore1 - transferAmount);
    assertEq(zoroBalanceAfter, zoroBalanceBefore + transferAmount);

    token.rebase(supplyDelta);

    uint256 luffyBalanceBefore2 = token.balanceOf(luffy);
    uint256 zoroBalanceBefore2 = token.balanceOf(zoro);
    token.transfer(zoro, transferAmount);
    uint256 luffyBalanceAfter2 = token.balanceOf(luffy);
    uint256 zoroBalanceAfter2 = token.balanceOf(zoro);
  ...
  function testTransferFromAfterRebase(
    uint256 transferAmount
  ) public {
    transferAmount = bound(transferAmount, 1e18, 10000e18);
    int256 supplyDelta = -9_000_000e18;
