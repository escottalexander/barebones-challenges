...
contract MultisendTest is Test {
  ...
  uint256 zoroExpectedBalance2 =
    defaultBalance + amounts[1] - amounts[0] - amounts[1];
  ...
  function testSendTokens() external {
    ...
    vm.expectEmit(true, true, false, true);
    emit Multisend.SuccessfulTokenTransfer(
      nami, tokenRecipients, amounts, address(dai)
    );
    multisend.sendTokens(tokenRecipients, amounts, address(dai));

    vm.expectEmit(true, true, false, true);
    emit Multisend.SuccessfulTokenTransfer(
      nami, tokenRecipients, amounts, address(weth)
    );
    ...
    vm.expectEmit(true, true, false, true);
    emit Multisend.SuccessfulTokenTransfer(
      zoro, tokenRecipients, amounts, address(dai)
    );
    multisend.sendTokens(tokenRecipients, amounts, address(dai));

    vm.expectEmit(true, true, false, true);
    emit Multisend.SuccessfulTokenTransfer(
      zoro, tokenRecipients, amounts, address(weth)
    );
    multisend.sendTokens(tokenRecipients, amounts, address(weth));
    vm.stopPrank;
  ...
  function testArraysNotEqualLength() external {
    ...
    vm.expectRevert();
    multisend.sendETH{ value: amounts[0] + amounts[1] }(
      recipients, biggerAmountsArray
    );
  ...
  function testUnsuccessfulETHTransfer() external {
    ...
    vm.etch(
      address(CONTRACT_NOT_PAYABLE),
      "By adding any arbitrary bytecode to the address, it will no longer be payable"
    );
